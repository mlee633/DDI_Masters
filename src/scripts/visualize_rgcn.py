...
visualize_rgcn.py
-----------------
Visualizes the multi-relational drug–drug graph used as input to the RGCN in MHD v3 Final.
Two main modes are possible:
  - Full graph sampling (subset of all nodes)
  - Top-k hub visualization (default)
Each edge colour corresponds to a specific relation type (ATC, CYP, or DDI).
...
def main():
    parser = argparse.ArgumentParser(description="Visualize the RGCN relation graph for MHD v3 Final.")
    parser.add_argument("--config", type=str, required=True, help="Path to YAML config file")
    parser.add_argument("--out", type=str, required=True, help="Path to save PNG")
    parser.add_argument("--top_k", type=int, default=10, help="Number of top hubs to visualize")
    args = parser.parse_args()
    ...
    print(f"[INFO] Relation types remapped to range 0–{num_rels-1}")
    ...
    # ---- Identify top-K hub drugs ----
    degree_dict = dict(G.degree())
    top_hubs = sorted(degree_dict, key=degree_dict.get, reverse=True)[:args.top_k]
    ...
    # Edge colouring by relation type
    edge_colors = [d["rel"] for _, _, d in H.edges(data=True)]
    cmap = plt.cm.get_cmap("tab10", num_rels)
    nx.draw_networkx_edges(H, pos, edge_color=edge_colors, edge_cmap=cmap, width=0.8, alpha=0.5, ax=ax)
    ...
    # Add colorbar for relation types
    sm = mpl.cm.ScalarMappable(cmap=cmap, norm=mpl.colors.Normalize(vmin=0, vmax=num_rels - 1))
    sm.set_array([])
    plt.colorbar(sm, ax=ax, fraction=0.03, pad=0.04, label="Relation Types")

    # Final formatting
    plt.title(f"Top-{args.top_k} Drug Hubs and Neighbors", fontsize=14)
    plt.axis("off")
    ...
    print(f"[INFO] Saved coloured RGCN visualization to {args.out}")
