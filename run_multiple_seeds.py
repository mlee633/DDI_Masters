...
CONFIG = "src/config/exp_mhd_v3_cold.yaml"
OUTPUT = Path("outputs")
...
def run_once(seed):
    ...
    subprocess.run([sys.executable, "-m", "src.train_mhd_v3", "--config", str(tmp_config)], check=True)

    # Find latest output folder
    subdirs = [p for p in OUTPUT.glob("*") if p.is_dir()]
